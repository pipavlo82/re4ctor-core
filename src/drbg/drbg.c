...
void re4_free(re4_ctx *ctx) { if (ctx) { memset(ctx,0,sizeof(*ctx)); free(ctx);} }
...
int re4_init(re4_ctx *ctx, const char *tag) {
  ...
  if (tag) {
    size_t L = strlen(tag); if (L > sizeof(ctx->tag)-1) L = sizeof(ctx->tag)-1;
    memcpy(ctx->tag, tag, L); ctx->tag[L]=0;
  }
